...
blogos_bp = Blueprint('blogos_bp', __name__, url_prefix='/brand')
base_api_url = "https://backend-api-1079186964678.us-central1.run.app"

#===Listado===
@blogos_bp.route('/<string:brand_id>/logos/')
@validar_sesion
def bklogos_main(brand_id):
	lista = listarLogos(brand_id)
	return render_template('sections/brand/logos/main.html',lista= lista,generar_url_firmada=generar_url_firmada)
#===Listado===
@blogos_bp.route('/<string:brand_id>/logos/list')
@validar_sesion
...
#===Save===
@blogos_bp.route("/<string:brand_id>/logos/save", methods=["GET", "POST"])
@validar_sesion
def bklogos_save(brand_id):
	if request.method == 'POST':
		...
		if data_info:
			...
			Assetsbrand.query.filter_by(asset_type='logos').update({'asset_estado': 0})
...
def gsfile_upload(file, uploader,brand_id):
	try:
		...
		nombre_archivo = f"logos/{name_fileext}"
...
def listarLogos(brand_id):
	try:
		query = Assetsbrand.query.filter_by(asset_type='logos',asset_estado=1,brand_id=brand_id)
		lista = query.all()
		return lista
	except Exception as e:

...
bfonts_bp = Blueprint('bfonts_bp', __name__, url_prefix='/brand')

#===Listado===
@bfonts_bp.route('/<string:brand_id>/fonts/')
@validar_sesion
def bkfonts_main(brand_id):
	lista = listarFonts(brand_id)
	return render_template('sections/brand/fonts/main.html',lista= lista)
#===Listado===
@bfonts_bp.route('/<string:brand_id>/fonts/list')
@validar_sesion
def bkfonts_list(brand_id):
	lista = listarFonts(brand_id)
	return render_template('sections/brand/fonts/list.html',lista= lista)
#===Save===
@bfonts_bp.route("/<string:brand_id>/fonts/save", methods=["GET", "POST"])
@validar_sesion
def bkfonts_save(brand_id):
	if request.method == 'POST':
		...
		if data_info:
			...
			Assetsbrand.query.filter_by(asset_type='fonts').update({'asset_estado': 0})
...
def gsfile_upload(file, uploader, brand_id):
	try:
		...
		nombre_archivo = f"fonts/{name_fileext}"
...
def generar_url_firmada(bucket_name, blob_name):
	...
	return url
def listarFonts(brand_id):
	try:
		query = Assetsbrand.query.filter_by(asset_type='fonts',asset_estado=1,brand_id=brand_id)
		lista = query.all()
		return lista
	except Exception as e:

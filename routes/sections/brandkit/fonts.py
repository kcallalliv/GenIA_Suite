...
bkfonts_bp = Blueprint('bkfonts_bp', __name__, url_prefix='/brand/<string:brand_id>/proyectos/<string:proyecto_id>/brandkit/fonts/')

#===Listado===
@bkfonts_bp.route('/')
@validar_sesion
def bkfonts_main(brand_id,proyecto_id):
	pid = request.values.get('pid')
	lista = listarFonts(pid)
	return render_template('sections/brandkit/fonts/main.html',lista= lista,generar_url_firmada=generar_url_firmada,pid=pid)
#===User Save===
@bkfonts_bp.route("save", methods=["GET", "POST"])
@validar_sesion
def bkfonts_save(brand_id,proyecto_id):
	if request.method == 'POST':
		...
		if data_info:
			...
			Assets.query.filter_by(asset_type='fonts').update({'asset_estado': 0})
...
def generar_url_firmada(bucket_name, blob_name):
	...
	return url
def listarFonts(data_id):
	try:
		query = Assets.query.filter_by(asset_type='fonts',asset_estado=1,proyecto_id=data_id)
		lista = query.all()
		return lista
	except Exception as e:

...
def bkimages_save():
	if request.method == 'POST':
		...
		if data_info:
			...
			Assets.query.filter_by(asset_type='files').update({'asset_estado': 0})
...
def generar_url_firmada(bucket_name, blob_name):
	...
	return url
def listarFiles(data_id):
	try:
		query = Assets.query.filter_by(asset_type='files',asset_estado=1,proyecto_id=data_id)
		lista = query.all()
		return lista

...
bklogos_bp = Blueprint('bklogos_bp', __name__)
...
#===Listado===
@bklogos_bp.route('/brandkit/logos')
def bklogos_main():
	...
	lista = listarLogos()
	return render_template('sections/brandkit/logos/main.html',logo=url_del_logo, lista= lista,generar_url_firmada=generar_url_firmada)
def generar_url_firmada(bucket_name, blob_name):
	...
	return url
def listarLogos():
	query = f"SELECT * FROM `{bq_table_assets}` WHERE asset_type = 'logos'"
	try:
...
#===User Save===
@bklogos_bp.route("/brandkit/logos/save", methods=["GET", "POST"])
def bklogos_save():

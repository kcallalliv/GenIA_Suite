...
tendencias_bp = Blueprint('tendencias_bp', __name__, url_prefix='/brand/<string:brand_id>/proyectos/<string:proyecto_id>/tendencias/')
#===View===
@tendencias_bp.route('/')
@validar_sesion
...
#===Create===
@tendencias_bp.route('keywords/', methods=['GET'])
@validar_sesion
def tendencias_keywords(brand_id,proyecto_id):
	if request.method == 'GET':
		return render_template('sections/tendencias/dashboard.html', proyecto_id=proyecto_id)
#===Save===
@tendencias_bp.route('keywords/addkeyword', methods=['POST'])
@validar_sesion
def keyword_save(brand_id,proyecto_id):
	if request.method == 'POST':
		...
		if encontrado:
			...
			return "success"
#===Listado Ajax===
@tendencias_bp.route('keywords/view-table', methods=['GET', 'POST'])
@validar_sesion
def tendencias_listarTableAjax(brand_id,proyecto_id):
	...
	try:
		if request.method in ['GET', 'POST']:
			...
			lista = query.all()
			if lista:
				return render_template('sections/tendencias/vista-table.html', lista=lista,pid=pid)
			else:
...
#===Listado Ajax Cards===
@tendencias_bp.route('/proyectos/view-cards', methods=['GET', 'POST'])
@validar_sesion
def tendencias_listarCardsAjax():
	try:
		if request.method in ['GET', 'POST']:
			...
			if listas:
				return render_template('sections/tendencias/vista-cards.html', listas=listas,convierteBase64=convierteBase64)
			else:
				return 'No se encontraron campañas con los criterios especificados.'
		return 'Error en el método de solicitud'
	except Exception as e:
...
#===Table Add===
#===============
@tendencias_bp.route('keywords/agregar', methods=['GET', 'POST'])
...
#===Select===
#============
@tendencias_bp.route('keywords/select-fuente', methods=['GET', 'POST'])

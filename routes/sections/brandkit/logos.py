...
@bklogos_bp.route('/')
@validar_sesion
def bklogos_main(brand_id,proyecto_id):
	#data_id = request.values.get('id')
	pid = proyecto_id
	lista = listarLogos(pid)
	return render_template('sections/brandkit/logos/main.html',lista= lista,generar_url_firmada=generar_url_firmada,pid=pid)
#===User Save===
@bklogos_bp.route("save", methods=["GET", "POST"])
@validar_sesion
def bklogos_save(brand_id,proyecto_id):
	if request.method == 'POST':
		...
		if data_info:
			...
			Assets.query.filter_by(asset_type='logos').update({'asset_estado': 0})
...
def listarLogos(data_id):
	try:
		query = Assets.query.filter_by(asset_type='logos',asset_estado=1,proyecto_id=data_id)
		lista = query.all()
		return lista
	except Exception as e:
